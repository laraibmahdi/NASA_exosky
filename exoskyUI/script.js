...
function initFirstScene() {
  ...
  renderer = new THREE.WebGLRenderer(
    { canvas: document.getElementById("starMap"), alpha: false },
    { antialias: true }
  );
...
window.onload = async function () {
  ...
  document
    ...
    .addEventListener("input", filterPlanets);
...
function loadNextBatch() {
  ...
  for (
    let i = loadedCount;
    i < Math.min(loadedCount + initallyLoadPlanets, planetsData.length);
    i++
  ) {
...
async function openSkySimulation(ra, dec, planetName) {
  ...
  function initPlanet(starData) {
    ...
    if (typeof OrbitControls !== "function") {
      console.error(
        "THREE.OrbitControls is not a constructor. Type:",
    ...
    planetTexture = textureLoader.load(
      getRandomTexture(),
      function (texture) {
  ...
  function onMouseMove(event) {
    ...
    const largeSphereGeometry = new THREE.SphereGeometry(
      interactionRadius,
      32,
      32
    );
